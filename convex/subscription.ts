...
const ENTITLED = new Set(["active", "trialing"])
 ...
 export const upsertFromPolar = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const base = {
            ...
            creditsGrantPerPeriod: args.creditsGrantPerPeriod ?? 
                existingByPolar?.creditsGrantPerPeriod ??
                existingByUser?.creditsGrantPerPeriod ??
                DEFAULT_GRANT,
            creditsRolloverLimit: args.creditsRolloverLimit ?? 
 ...
 export const grantCreditsIfNeeded = mutation({
    ...
    handler: async (ctx, {subscriptionId, idempotencyKey, amount, reason}) => {
        ...
        if (grant <= 0) {
            return { ok: true, skipped: true, reason: "no-credits-to-grant" }
        }
 ...
 export const consumeCredits = mutation({
    ...
    handler: async (ctx, {userId, amount, reason, idempotencyKey}) => {
        ...
        if (idempotencyKey) {
            ...
            if (dupe) return { ok: true, idempotent: true, reason: "duplicate-ledger" }
        }
        ...
        if (!sub) return { ok: false, error: "subscription-not-found" }
        if (!ENTITLED.has(sub.status)) return { ok: true, skipped: true, reason: "not-entitled" }
