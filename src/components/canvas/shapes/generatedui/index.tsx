...
const GeneratedUI = ({shape, toggleChat, generateWorkflow, exportDesign}: Props) => {
    ...
    const handleExportDesign = () => {
        if (!shape.uiSpecData) {
            console.warn("no UI spec data to export")
            return
        }
  ...
  return (
    ...
    style={{
        ...
        style={{
            ...
            style={{
                ...
                <div className='absolute -top-8 right-0 flex gap-2'>
                    <LiquidGlassButton size='sm' variant='subtle' onClick={handleExportDesign} disabled={!shape.uiSpecData} 
                    style={{pointerEvents: "auto"}}>
                        <Download size={12}/>
                        Export
                    </LiquidGlassButton>

                    <LiquidGlassButton size='sm' variant='subtle' onClick={handleGenerateWorkflow} disabled={!shape.uiSpecData} 
                    style={{pointerEvents: "auto"}}>
                        <Workflow size={12}/>
                        GenerateWorkflow
                    </LiquidGlassButton>

                    <LiquidGlassButton size='sm' variant='subtle' onClick={handleToggleChat} disabled={!shape.uiSpecData} 
                    style={{pointerEvents: "auto"}}>
        ...
        <div className='absolute -top-6 left-0 text-xs px-2 py-1 rounded whitespace-nowrap text-white/60 bg-black/40'
        style={{fontSize: "10px"}}>
            Generated UI
        </div>
    </div>
